...
let example2 = If (
  Not (PBool(1)),       (* not b1 *)
  While (And (PBool(2),One), Pact("b1")), (* then while b2 and 1 do b1*)
  Pact("b2")             
  )
...
(* b1 + 1 *)
let exp2 = from_BE_to_KAT (Or (PBool(1),One))


let test1 =   While(                   
PBool(2),           
((Seq (Test(PBool(1)), Pact("b1")))))
...
let e1 = 
  While (
    Or(PBool(1),PBool(1)), 
    While((And(PBool(1),PBool(2))),Pact("p0"))
  )
