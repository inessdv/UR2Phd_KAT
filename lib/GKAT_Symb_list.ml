...
module BExp = struct
  ...
  let b_not (b1 : t) : t =
    if b1 == one then zero else if b1 == zero then one else [Not b1] (** ????**)
...
module Exp = struct
  ...
  let p_act (p : string) : t = [Pact p]
  let test (b : BExp.t) : t = [Test b]
  ...
  let while_do (b : BExp.t) (e : t) : t = [While (b, e)]
end

module Derivatives = struct
  (** defines derivatives *)

  module DSet = Set.Make (String)
  ...
  let sequence_helper_with_epsilon (eps : BExp.t)
    ...
    List.fold_left
      ...
      let rec derivative (exp : Exp.t) : ((BExp.t_ * Exp.t) * string) list =
        match exp.node with
        | Test _ -> []
...
let product (psi_e:(BExp.t_ Hashcons.hash_consed * (Exp.t * string)) list) (psi_f:(BExp.t_ Hashcons.hash_consed * (Exp.t * string)) list): ((BExp.t_ Hashcons.hash_consed * (Exp.t * string)) * (BExp.t_ Hashcons.hash_consed * (Exp.t * string))) list =
  ...
  let rec reject (exp : Exp.t) : BExp.t =
    ...
    let transitions = List.fold_left (fun acc (be,(_,_)) -> acc || be ) false exp_derivatives in
      _
