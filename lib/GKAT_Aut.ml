...
let rec thompson_construct (exp : gkat) (p_act : PActSet.t)
  ...
  | Seq (ex1, ex2) ->
      let auto1 = thompson_construct ex1 p_act p_test in
      let auto2 = thompson_construct ex2 p_act p_test in
      ...
      (* let right_states =State.Set.map coprod.to_right s2.states in  *)
      {
        ...
        trans =
          (fun state atom ->
            ...
            | Right s -> auto2.trans s atom |> res_to_right
            | Left s -> (
                match auto1.trans s atom with
                | Accept ->
                    auto2.p_start p_test |> res_to_right (*Not important input*)
                | r -> r |> res_to_left));
        p_start =
          (fun at ->
  ...
  | If (bexp, exp1, exp2) ->
        ...
        | To (s, p_act) -> To (coprod.to_right s, p_act) in
      {
        ...
        trans = (fun state atom ->
          match coprod.from_coprod state with
          | Right s -> auto2.trans s atom |> res_to_right
          | Left s -> auto1.trans s atom |> res_to_left);
        p_start = (fun atom -> if satisfy atom bexp then auto1.p_start atom |> res_to_left else auto2.p_start atom |> res_to_right);
      }
